...
**Implementation progress:** 1 task complete
**Next:** Task 2 - OS registry + validation

This is the canonical task ledger for Dashboard, like Data's 41-task map.

## Phase 1 - Read-only Control Room (tasks 1-8)

- [x] Task 1 - Protocol package (versioned envelope, systemId + workspaceId, Zod, no raw roots) — DONE 2026-09-12: packages/protocol/src (ids, methods, envelope) + test/protocol.test.ts 9/9, npm run check green
- [ ] Task 2 - OS registry + validation (compatible OS check, systemId map, per-window selection)
